...
int main(int argc, char * argv[]) {
  ...
  if ((error = parse_options(argc, argv, NULL, ext_longopts,
	...
	for (unsigned int i = 0; i < num_params; i++) {
        ...
        double max_priors[] = {1.0e+6,1.0e+6,1.0e+6, 1.0e+10};
        for (unsigned int i = 0; i < num_params; i++) {
    ...
    if (rank == 0) {
      if (optind + 1 < argc) {
        ...
        const char* extension = "_b.hdf5";
        name_with_extension = malloc(strlen(argv[optind++])+1+7);
        strcpy(name_with_extension,argv[optind]);
        strcat(name_with_extension, extension);

        if ((error = gmcmc_filewriter_create_hdf5(&mcmc_options.burn_in_writer,
                                                name_with_extension, num_params, 1, 
      ...
      const char* extensionp = "_p.hdf5";
      char* name_with_extension_p;
      name_with_extension_p = malloc(strlen(argv[optind])+1+7);
      strcpy(name_with_extension_p,argv[optind]);
      strcat(name_with_extension_p, extensionp);

      if ((error = gmcmc_filewriter_create_hdf5(&mcmc_options.posterior_writer,
                                              name_with_extension_p, num_params, 1, 
                                              mcmc_options.num_temperatures,
                                              mcmc_options.num_posterior_samples)) != 0) {

        
        free(temperatures);
        free(name_with_extension_p);
        gmcmc_burst_dataset_destroy(dataset);
        gmcmc_model_destroy(model);
        gmcmc_ion_model_destroy(ion_model);
     ...
     */
    error = gmcmc_popmcmc_mpi(model, dataset, gmcmc_ion_missed_events_likelihood_mh, gmcmc_proposal_mh, &mcmc_options, rng);
...
static void precondition_covariance(double * M, size_t ldq) {
    if (extra.precondition ){
        ...
        M[0 * ldq + 0] = 0.0055912807361189;
        M[1 * ldq + 0] = -0.0002032110640672;
        M[2 * ldq + 0] = 0.0001425139934387;
        M[3 * ldq + 0] = -0.0000000003665896;

        M[0 * ldq + 1] =-0.0002032110640672 ;
        M[1 * ldq + 1] = 0.0000087649141186;
        M[2 * ldq + 1] = -0.0000040060242212;
        M[3 * ldq + 1] = 0.0000000005251729;
        
        M[0 * ldq + 2] = 0.0001425139934387;
        M[1 * ldq + 2] = -0.0000040060242212;
        M[2 * ldq + 2] = 0.0000390741523190;
        M[3 * ldq + 2] = -0.0000000018166074;
        
        M[0 * ldq + 3] = -0.0000000003665896;
        M[1 * ldq + 3] = 0.0000000005251729;
        M[2 * ldq + 3] = -0.0000000018166074;
        M[3 * ldq + 3] = 0.0000000000016076;
