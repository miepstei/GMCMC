...
int main(int argc, char * argv[]) {
    ...
    const char * data_file = "data/Ball_Burst_Data_Single.mat";
  ...
  if ((error = parse_options(argc, argv, NULL, ext_longopts,
     ...
     * Model settings
     */
    const unsigned int num_params = 2;
    ...
    if(extra.log10space){
        ...
        double max_priors[] = {6 , 12};
	for (unsigned int i = 0; i < num_params; i++) {
	    if ((error = gmcmc_distribution_create_uniform(&priors[i], 0, max_priors[i])) != 0) {
	        // Clean up
        ...
        double max_priors[] = {1.0e+6  , 1.0e+12};
        for (unsigned int i = 0; i < num_params; i++) {
            if ((error = gmcmc_distribution_create_uniform(&priors[i], 1, max_priors[i])) != 0) {
                // Clean up
                for (unsigned int j = 0; j < i; j++)
                    gmcmc_distribution_destroy(priors[i]);
    ...
    if (extra.log10space) {
        const double params[] = { 3.6847, 7.68 };
        ...
        const double params[] = { 1000.0, 5.473949172720000e+05 };
        if ((error = gmcmc_model_set_params(model, params)) != 0) {
            // Clean up
            free(temperatures);
            gmcmc_burst_dataset_destroy(dataset);
    ...
    if ((error = gmcmc_ion_model_create(&ion_model, 1, 1, calculate_Q_matrix , precondition_covariance)) != 0) {
        // Clean up
        for (unsigned int i = 0; i < num_params; i++)
            gmcmc_distribution_destroy(priors[i]);
        free(priors);
...
static void calculate_Q_matrix(const double * params, double * Q, size_t ldq, double conc){//, double conc) {
    ...
    // Q_AA - 1 by 1
    Q[0 * ldq + 0] =  -(Alpha_2);
    Q[0 * ldq + 1] =  Beta_2 * conc;
    Q[1 * ldq + 0] =  Alpha_2;
    Q[1 * ldq + 1] =  -(Beta_2) * conc;
}

static void precondition_covariance(double * M, size_t ldq) {
    if (extra.precondition ){
        //return preconditioned matrix covariance^-1
        M[0 * ldq + 0] = 0.0013174594514434;
        M[0 * ldq + 1] = -0.0000000049248691;
        M[1 * ldq + 0] = -0.0000000049248691;
        M[1 * ldq + 1] = 0.0000000000145237;
    } else {
