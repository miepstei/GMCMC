...
int main(int argc, char * argv[]) {
    ...
    const char * data_file = "data/Ball_Burst_Data.mat";
  ...
  if ((error = parse_options(argc, argv, NULL, ext_longopts,
	...
	for (unsigned int i = 0; i < num_params; i++) {
        ...
        double max_priors[] = {1.0e+5  , 1.0e+8};
        for (unsigned int i = 0; i < num_params; i++) {
    ...
    if (extra.log10space) {
        const double params[] = { 2, 5.738300760536707 };
        ...
        const double params[] = { 100.0, 5.473949172720000e+05 };
        if ((error = gmcmc_model_set_params(model, params)) != 0) {
            // Clean up
            free(temperatures);
            gmcmc_burst_dataset_destroy(dataset);
